...
2. ไปที่โฟลเดอร์ backend ของโปรเจกต์:
   ...
   ```bash
   cd /Users/pmykingg/Documents/CMUShareCycle_KingNicety/Untitled/backend
   ```
...
1. ถ้ารัน backend อยู่ (เช่น `npm start` หรือ `./start-backend.sh`) ให้กด **Ctrl+C** หยุด
2. รันใหม่:

   ```bash
   npm start
   ```
   หรือจาก root โปรเจกต์: `./start-backend.sh`
