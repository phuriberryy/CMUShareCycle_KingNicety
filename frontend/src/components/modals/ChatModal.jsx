...
export default function ChatModal({ open, onClose, initialChatId, asPage = false }) {
  ...
  }, [sheetOpen, pendingImage, composerText])
  ...
  const AttachmentSheet = ({ onCloseSheet }) => (
    <div className={`fixed inset-0 z-50 transition-opacity ${sheetOpen ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0'}`}>
      <div className="absolute inset-0 bg-black/30" onClick={onCloseSheet} />
      <div className={`absolute inset-x-0 bottom-0 translate-y-0 transition-transform duration-300 ease-out ${sheetOpen ? 'translate-y-0' : 'translate-y-full'}`}>
        <div className="mx-auto max-w-2xl rounded-t-3xl border border-gray-200 bg-white p-4 shadow-2xl">
          <div className="mx-auto mb-3 h-1.5 w-12 rounded-full bg-gray-200" />
          <button type="button" onClick={() => { handlePickImage(); onCloseSheet() }} className="flex min-h-12 w-full items-center gap-3 rounded-2xl px-4 py-3 text-left text-sm font-medium text-gray-800 transition active:scale-[0.98] hover:bg-gray-50">
            <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-primary/10 text-primary"><ImageIcon size={18} /></span>
            Upload image
          </button>
          <button type="button" onClick={() => { handlePickImage(); onCloseSheet() }} className="mt-2 flex min-h-12 w-full items-center gap-3 rounded-2xl px-4 py-3 text-left text-sm font-medium text-gray-800 transition active:scale-[0.98] hover:bg-gray-50">
            <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-600"><Camera size={18} /></span>
            Open camera
          </button>
          <button type="button" onClick={onCloseSheet} className="mt-2 flex min-h-12 w-full items-center justify-center rounded-2xl bg-gray-100 px-4 py-3 text-sm font-semibold text-gray-700 transition active:scale-[0.98]">Close</button>
        </div>
      </div>
    </div>
  )

  const MessageList = ({ compactPadding = false }) => (
    <div ref={messageScrollRef} className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-4 md:px-5 md:py-5" style={{ paddingBottom: `calc(${composerHeight}px + env(safe-area-inset-bottom) + ${compactPadding ? '0.75rem' : '1rem'})` }}>
          ...
          {groupedMessages.map((message) => {
            ...
            return (
              <div key={getMessageId(message)} className={`flex ${mine ? 'justify-end' : 'justify-start'} ${message._groupStart ? 'mt-3' : 'mt-0.5'}`}>
                <div className={`max-w-[82%] md:max-w-[72%] rounded-2xl px-3 py-2 text-sm shadow-sm transition-opacity ${mine ? 'bg-primary text-white' : 'bg-white text-gray-800 border border-gray-200'} ${message.pending ? 'opacity-70' : 'opacity-100'}`}>
  ...
  const Composer = () => (
    <div ref={composerRef} className="shrink-0 border-t border-gray-100 bg-white px-4 py-3 shadow-[0_-8px_24px_rgba(0,0,0,0.04)] md:px-5">
  ...
  const mobileDetail = (
    <div className={`flex h-full min-h-0 flex-col bg-[#FBFCFB] md:hidden ${isMobileChatDetail ? 'flex' : 'hidden'}`}>
      ...
      <MessageList compactPadding />
      <Composer />
      <AttachmentSheet onCloseSheet={() => setSheetOpen(false)} />
    </div>
  )
        ...
        {activeChat ? (
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
            <div className="shrink-0 border-b border-gray-200 bg-white/95 px-5 py-4 backdrop-blur-sm">
              <div className="flex items-center gap-3">
                <button type="button" onClick={onClose} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-gray-100 text-gray-600 transition hover:bg-gray-200" aria-label="ปิดแชท">
                  <X size={18} />
                </button>
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-primary text-sm font-bold text-white shadow-sm">{(activeChat?.participant_name || 'CMU').split(' ').map((p) => p[0]).join('').slice(0, 2).toUpperCase()}</div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <p className="truncate text-lg font-semibold text-gray-900">{activeChat?.participant_name || 'นักศึกษา CMU'}</p>
                    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-semibold ${socketConnected ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>{socketConnected ? 'Online' : 'Connecting'}</span>
                  </div>
                  <p className="truncate text-xs text-gray-500">{activeChat?.participant_email || ''}</p>
                </div>
              </div>
