import { CheckCircle, Clock, Gift, Loader2, X } from 'lucide-react'

/**
 * Persistent donation-status banner pinned at the top of a donation chat room.
 * Mirrors ExchangeBanner — same positioning, animation, and state-transition
 * logic — adapted for the donation hand-off lifecycle.
 *
 * Props
 * ─────
 * chat        – full chat object from mapChatRow (must have isDonationChat=true)
 * onConfirm   – async fn; called when the current user confirms the hand-off
 * onAccept    – async fn; called when the current user accepts a pending donation
...
export default function DonationBanner({
  ...
  if (!chat?.isDonationChat) return null
      ...
      <div className="sticky top-0 z-20 mx-3 mt-3 mb-1 overflow-hidden rounded-2xl border border-green-200 bg-green-50 shadow-sm">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[13px] font-bold text-green-800">ส่งต่อสำเร็จแล้ว!</p>
            {chat.itemTitle ? (
              <p className="truncate text-[11px] text-green-600">"{chat.itemTitle}" — สิ่งของชิ้นนี้ถูกส่งต่อเรียบร้อยแล้ว</p>
            ) : null}
          </div>
  ...
  // ─── Active state – waiting for delivery confirmations ────────────────────────
  if (chat.status === 'active') {
    const oneConfirmed = myConfirmed || otherConfirmed
    const headerText   = oneConfirmed ? 'กำลังรอการยืนยันจากอีกฝ่าย' : 'พร้อมนัดหมายการรับสิ่งของ'
    ...
    return (
      <div className="sticky top-0 z-20 mx-3 mt-3 mb-1 overflow-hidden rounded-2xl border border-teal-200 bg-teal-50/90 shadow-sm backdrop-blur-sm">
        {/* Header row */}
        <div className="flex items-center gap-3 border-b border-teal-100 px-4 py-2.5">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[12px] font-bold text-teal-900">{headerText}</p>
            {!oneConfirmed ? (
              <p className="truncate text-[11px] text-teal-700">พูดคุยเพื่อนัดหมายวัน เวลา และสถานที่รับสิ่งของได้เลย</p>
            ) : null}
          </div>
          {/* Progress pills */}
          <div className="flex shrink-0 items-center gap-1.5">
            <ProgressPill label={isOwner ? 'คุณ' : 'ผู้บริจาค'} done={isOwner ? myConfirmed : otherConfirmed} />
            <ProgressPill label={isOwner ? 'ผู้รับ' : 'คุณ'}    done={isOwner ? otherConfirmed : myConfirmed} />
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 px-4 py-2.5">
          <div className="min-w-0 flex-1">
            <p className="text-[11px] leading-snug text-teal-800">{statusText}</p>
            {!myConfirmed && !otherConfirmed ? (
              <p className="mt-0.5 text-[10px] text-teal-600">กดหลังจากส่งมอบสิ่งของเรียบร้อยแล้ว</p>
            ) : null}
          </div>
          {!myConfirmed ? (
            <button
              ...
              onClick={onConfirm}
              disabled={confirming}
              className="inline-flex h-8 shrink-0 items-center gap-1.5 rounded-xl bg-teal-500 px-3 text-[12px] font-bold text-white shadow-sm transition hover:bg-teal-600 active:scale-95 disabled:opacity-60"
            >
              {confirming ? (
  ...
  // ─── Pending state – waiting for donation request to be mutually accepted ──────
  if (chat.status === 'pending') {
    ...
    if (myAccepted && otherAccepted) return null // rare transient state
    ...
    return (
      <div className="sticky top-0 z-20 mx-3 mt-3 mb-1 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div className="flex items-center gap-3 border-b border-gray-100 px-4 py-2.5">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[12px] font-bold text-gray-800">คำขอรับบริจาค</p>
            {chat.itemTitle ? (
              <p className="truncate text-[11px] text-gray-500">"{chat.itemTitle}"</p>
            ) : null}
          </div>
          <div className="flex shrink-0 items-center gap-1.5">
            <ProgressPill label={isOwner ? 'คุณ' : 'ผู้บริจาค'} done={isOwner ? myAccepted : otherAccepted} />
            <ProgressPill label={isOwner ? 'ผู้รับ' : 'คุณ'}    done={isOwner ? otherAccepted : myAccepted} />
          </div>
        </div>
        ...
        {!myAccepted ? (
          <div className="flex items-center gap-2 px-4 py-2.5">
            <p className="min-w-0 flex-1 text-[11px] text-gray-600">
              {otherAccepted ? 'อีกฝ่ายตกลงแล้ว — คุณต้องการยืนยันการบริจาคนี้ไหม?' : 'รอการยืนยันจากทั้งสองฝ่าย'}
            </p>
            <button
              type="button"
              onClick={onDecline}
...
// ─── Sub-components (shared visual language with ExchangeBanner) ──────────────
