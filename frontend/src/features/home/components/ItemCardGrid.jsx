        ...
        return (
          ...
          >
            <div className="flex items-center gap-2 bg-white px-3 py-1.5 sm:gap-2.5 sm:border-b sm:border-gray-50 sm:bg-gradient-to-r sm:from-gray-50/80 sm:to-white sm:px-3.5 sm:py-2">
              <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-primary-light text-[9px] font-bold tracking-tight text-primary-dark ring-2 ring-white shadow-sm sm:h-8 sm:w-8 sm:text-[11px]">
                {ownerInitials(owner)}
              </div>
              <div className="min-w-0 flex-1">
                <p className="truncate text-[12px] font-semibold text-gray-900 sm:text-[13px]">{owner}</p>
                <p className="truncate text-[11px] text-gray-500 sm:text-[11px]">{faculty || 'มหาวิทยาลัยเชียงใหม่'}</p>
              </div>
              <button
                type="button"
            ...
            <div className="relative aspect-[16/9] w-full overflow-hidden bg-gray-100">
              ...
              <div className="absolute inset-x-0 top-0 flex items-start justify-between gap-2 p-2 sm:p-2.5">
                {daysLabel ? (
                  <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold shadow-sm backdrop-blur-[2px] sm:px-2.5 sm:text-[11px] ${daysLabel.style}`}>
                ...
                <span
                  className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold text-white shadow-md sm:px-2.5 sm:text-[11px] ${
            ...
            <div className="flex min-h-0 flex-col gap-1.5 p-3 pt-2 sm:flex-1 sm:gap-1.5 sm:p-3 sm:pt-2">
              <div className="flex flex-wrap items-center gap-1.5">
                <span className="inline-flex max-w-full truncate rounded-full bg-primary-light/70 px-2 py-0.5 text-[10px] font-semibold text-primary-dark ring-1 ring-primary/10 sm:text-[10.5px]">
                  #{getCategoryLabel(item.category, item.other_subtype)}
                </span>
                <span className="rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-semibold text-gray-700 ring-1 ring-gray-200/80 sm:text-[10.5px]">
              ...
              <h3
                onClick={() => navigate(`/items/${item.id}`)}
                className="line-clamp-1 cursor-pointer text-[13px] font-bold leading-snug tracking-tight text-gray-900 transition hover:text-primary sm:text-[14.5px]"
              >
                {item.title}
              </h3>

              <div className="flex min-h-0 flex-col text-xs text-gray-500 sm:flex-1">
                <div className="hidden min-h-0 space-y-1 sm:block">
                  {expiryLine ? (
                    <p className="flex items-center gap-1.5 text-[11.5px] font-medium text-gray-600">
                  ...
                  <p className="hidden items-center gap-1.5 text-[11.5px] sm:flex">
                    <UserIcon size={12} className="shrink-0 text-primary/70" />
                    <span className="truncate font-medium text-gray-600">{owner}</span>
                  </p>
                </div>
                <div className="flex shrink-0 gap-2 pt-1.5 sm:mt-auto sm:pt-2.5">
                  <button
                    type="button"
                    onClick={() => navigate(`/items/${item.id}`)}
                  ...
                  {item.status === 'active' && item.listing_type === 'donation' ? (
                    <button
                      ...
                      className="hidden h-[30px] flex-1 items-center justify-center gap-1.5 rounded-lg bg-rose-500 px-2.5 text-[12px] font-semibold text-white shadow-sm transition hover:bg-rose-600 sm:inline-flex"
                    >
                    ...
                    <button
                      ...
                      className="hidden h-[30px] flex-1 items-center justify-center gap-1.5 rounded-lg bg-primary px-2.5 text-[12px] font-semibold text-white shadow-sm ring-1 ring-white/20 transition hover:bg-primary-dark sm:inline-flex"
                    >
                      <RefreshCcw size={14} className="shrink-0" />
                      <span className="whitespace-nowrap">ขอแลกเปลี่ยน</span>
                    </button>
                  ) : (
                    <div className="hidden h-[30px] flex-1 cursor-default items-center justify-center rounded-lg border border-dashed border-gray-200 bg-gray-50 px-3 text-[12px] font-medium text-gray-500 sm:flex">
                      {primaryActionLabel}
                    </div>
                  )}
                  <button
                    type="button"
                    onClick={() => navigate(`/items/${item.id}`)}
                    className="sc-btn-secondary hidden h-[30px] flex-1 items-center justify-center gap-1.5 rounded-lg px-2.5 text-[12px] font-semibold sm:inline-flex"
                  >
