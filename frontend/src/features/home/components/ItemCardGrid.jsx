...
export default function ItemCardGrid({ items, navigate, onExchangeItem, onDonationItem }) {
  return (
    <div className="mx-auto grid w-full max-w-[1100px] grid-cols-1 gap-4 px-0 sm:grid-cols-2 sm:gap-5 lg:auto-rows-fr lg:grid-cols-[repeat(auto-fill,minmax(280px,1fr))] lg:gap-5 lg:px-6 xl:px-6">
      {items.map((item) => {
        ...
        return (
          <article
            key={item.id}
            className={`group flex h-full min-h-[520px] w-full flex-col overflow-hidden rounded-[18px] border border-black/[0.05] bg-white shadow-sm ring-0 transition duration-200 sm:rounded-[18px] lg:min-h-[unset] ${
          ...
          >
            ...
            <div className="flex min-h-0 flex-1 flex-col gap-2 p-3 pt-2.5 sm:gap-3 sm:p-4 sm:pt-3 lg:gap-2.5 lg:p-4 lg:pt-3.5">
              ...
              <h3
                onClick={() => navigate(`/items/${item.id}`)}
                className="line-clamp-2 min-h-[2.75rem] cursor-pointer text-sm font-bold leading-snug tracking-tight text-gray-900 transition hover:text-primary sm:min-h-[3rem] sm:text-[15px]"
              >
                {item.title}
              </h3>

              <div className="flex min-h-0 flex-1 flex-col border-t border-gray-50 pt-2 text-xs text-gray-500 sm:pt-2.5">
                <div className="min-h-0 space-y-1 sm:space-y-1.5 lg:space-y-1.5">
                  {expiryLine ? (
                ...
                <div className="mt-auto flex shrink-0 gap-2 pt-2.5 sm:pt-3 lg:gap-2 lg:pt-3.5">
                  <button
                    type="button"
                    onClick={() => navigate(`/items/${item.id}`)}
                  ...
                  {item.status === 'active' && item.listing_type === 'donation' ? (
                    <button
                      ...
                      className="hidden h-[34px] flex-1 items-center justify-center rounded-lg bg-rose-500 px-3 text-[13px] font-semibold text-white shadow-sm transition hover:bg-rose-600 sm:inline-flex"
                    >
                      <Heart size={18} />
                      ขอรับบริจาค
                    </button>
                    ...
                    <button
                      ...
                      className="hidden h-[34px] flex-1 items-center justify-center rounded-lg bg-primary px-3 text-[13px] font-semibold text-white shadow-sm ring-1 ring-white/20 transition hover:bg-primary-dark sm:inline-flex"
                    >
                      <RefreshCcw size={18} />
                      ขอแลกเปลี่ยน
                    </button>
                  ...
                  <button
                    type="button"
                    onClick={() => navigate(`/items/${item.id}`)}
                    className="sc-btn-secondary hidden h-[34px] flex-1 rounded-lg px-3 text-[13px] font-semibold sm:mt-0 sm:inline-flex"
                  >
                    <Eye size={18} />
                    ดูรายละเอียด
                  </button>
